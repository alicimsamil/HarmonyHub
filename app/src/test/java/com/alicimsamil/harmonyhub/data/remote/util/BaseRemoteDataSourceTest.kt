...
class BaseRemoteDataSourceTest {
    ...
    fun perform_network_call_should_return_error_with_correct_message_for_400_codes() = runTest {
        val result = dataSource.performApiCall { Response.error<String>(400, ResponseBody.create(MediaType.parse("text/plain"),"Client error")) }
    ...
    fun perform_network_call_should_return_error_with_correct_message_for_500_codes() = runTest {
        val result = dataSource.performApiCall { Response.error<String>(500, ResponseBody.create(MediaType.parse("text/plain"),"Server error")) }
        assertTrue(result is Error)
        result.onFailure {
